...
- **Admin-only actions** — the five management ones (`access.staff.invite`,
  ...
  `access.customer.delete`, amendment 43) and `access.staff_settings.update`, the numbers that
  decide how staff sign in (amendment 46(a)) — can be put only into admin roles. Staff manage no
  roles and no people, and change no rule of signing in. A **store's own** settings — its terms
  version, how many addresses, its verification numbers — stay an ordinary action,
...
- **Nobody works without a role** (amendment 27, as amendment 45(b) settled it). There is no
...
- **A staff member's stores** are the store row chosen for them, plus any store an exception adds.
  ...
  manages them. "Centralized" is not stored anywhere: it is what the screens call a staff member
  whose stores number more than one, read from the assignment.
...
- **Removed only by a console command:** `php artisan access:super-admin:revoke {email}` takes the
  power away and **closes the account** with it (amendment 45(b)): `CANCELLED`, no password, every
  link, code and session gone, and the email and phone free at once for a new account. Someone who
  is to stay is invited again like anyone else. An invited Super Admin who never accepted is
  cancelled the same way, and freed (amendment 30). **The last
...
- Every address also has a `label` ("Home"), the `recipient_name`, the recipient's `phone` (E.164,
  ...
  in every store**, with **one default** per store — at most **10** (amendment 41), a per-store
  setting. The first address saved in a store becomes its default; deleting the default moves the
  flag to the newest remaining address.
...
- **Store address formats** are data: the fields (key, labels in both languages, required,
  ...
  (a new country, before its scheme is entered) refuses addresses with a clear error. Every store
  starts with the scheme above — written when the schema is migrated, and when a store is opened
  later (amendment 41) — so nothing waits for the staff screen. A value whose key the format does
  not define is refused. The template is plain text with `{field}` placeholders: an empty one
  disappears, and a line left empty is dropped.
- **A format that changes later leaves saved addresses alone**, but an address that no longer
  satisfies its store's format — a field asked for, shortened or dropped — cannot be used for an
  order until the customer completes it (`AddressDto.isComplete`, amendments 41 and 42).
- **Every value is text on one line** (amendment 42): what a customer types reaches a shipping
  label, so control characters are refused; a form holds at most 60 fields and one address 4,000
  characters in all.
...
- The customer asks from their account page, confirming with their **password**. Staff can do it on
  the customer's request, under an **admin-only** action that records a reason (amendment 43); the
  same action cancels a deletion for a customer who cannot sign in.
- The account is **locked at once** (it cannot order), **every session of theirs ends at once**
  (amendment 45: they are signed out of every device the moment they confirm), and it is
  **anonymized 14 days later**. The customer is told once, when it is scheduled: the date, and that
  signing in cancels it (amendment 43).
...
| `RequestAccountDeletion` — confirmed with the password; signs them out everywhere (amendment 45) | every customer | `access.account.delete` | Global |
...
| A **store's** settings — terms version, addresses per store, its verification and OTP numbers (Platform's `UpdateSetting`, under Access's permission) | role | `access.settings.update` | That store |
| The **staff security** settings — password length, lockouts, session limits, code length | role, **admin-only** (amendment 46(a)) | `access.staff_settings.update` | All stores (they are global) |
...
| Table | Columns |
|---|---|
| `access.phone_codes` | `customer_id` PK/FK, `purpose` (`ADD`, `CHANGE`), `phone`, `code_hash`, `attempts`, `expires_at`, `sent_at` — one live code per customer |
...
| `access.role_assignment_stores` | (`staff_user_id`, `store_id`) PK, FK → `platform.stores` — rows exist exactly when `SELECTED_STORES` (code rule: `RoleAssignment` refuses any other shape; the database holds the keys only) |
...
| `access.role_assignment_exception_stores` | (`staff_user_id`, `permission`, `store_id`) PK, FK → `platform.stores` — rows exist exactly when the exception is `SELECTED_STORES` (code rule, as above) |
...
├── InvalidAddress                INVALID      a field fails the store's format, or is not in it (amendment 41)
├── TooManyAddresses              CONFLICT     more than the store allows for one customer (amendment 41)
...
a Super Admin; and since amendment 45(b) the case no longer arises for a revoked Super Admin, whose
account is closed outright (owner, 2026-09-19 and 2026-09-20; amendments 27 and 45). #36, whether staff holding `access.staff.view` see admins — they see an
admin's name and role only, and a Super Admin not at all (owner, 2026-09-20; amendment 43).
...
| 46 | §1.4, §1.5, §1.10, §3.3, §5.6 | **From the three independent reviews of the whole module, and the owner's answers** (2026-09-21): (a) the settings permission is **split in two**: the staff security numbers — password length, lockouts, session limits, code length — move to `access.staff_settings.update`, which is **admin-only** like the other actions that reach every account, while a store's own settings (terms version, addresses per store, verification numbers) stay `access.settings.update`, an ordinary action a staff role may hold; (b) a staff member changing **their own phone gives their current password first**, counted like a wrong one at sign-in: the number is where the sign-in code goes, so a stolen session alone must not move the second factor; (c) **anonymizing takes the account's session rows with it** — each row holds the person's id, address and browser for up to a year — so the storefront's session rows now carry the customer they belong to; (d) an **admin's status** is not part of "a name and a role" either (amendments 43(a), 44(e)): an ordinary staff member does not learn whether a colleague's account is active, nor their joining date — so for a reader who is not a Super Admin the staff list is ordered **by name**, because a list ordered by joining date brackets an admin between the colleagues whose dates that reader does see; (e) the admin "forgot password" page keeps **no per-address limit**: the reset is already capped at 3 an hour per account and the case is rare; (f) the rule that one email belongs to one account is now enforced **both ways** — inviting a staff member, changing a staff email, or creating a Super Admin from the console, onto an address a customer holds, is refused in the same words as a colleague's; (g) the SMS limiter keys hold a **hash** of the number, never the number; the sweep runs only from the console or the queue and stops when a round moves nothing; `CustomerDto` says whether an account was anonymized; a staff sign-in leaves no session behind a rolled-back transaction; the map pin is audited as personal. | The module's last pass before it merges. (a)–(d) are the owner's answers to what the reviews raised; the rest are defects the reviews found in the code, not changes of rule. | Owner and the reviews of step 7, 2026-09-21 |
