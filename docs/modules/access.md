...
- **A format that changes later leaves saved addresses alone**, but an address that no longer
  satisfies its store's format cannot be used for an order until the customer completes it
  (`AddressDto.isComplete`, amendment 41).
