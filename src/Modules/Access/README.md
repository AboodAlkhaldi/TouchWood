...
| `Application/Query` | `ListRoles`, `ViewRole`, `RoleEditorPermissions`, `MyPermissions`, `ListCustomers`, `ViewCustomer`, `ListStaff`, `ViewStaff`, and the `RoleReader`, `CustomerReader` and `StaffReader` they use, with `StaffVisibility` (who a reader may see) and `StoreIds` (the reader's stores, as ids). |
...
things at once queue up instead of deadlocking; every handler that writes a role, a staff member or
an assignment still retries a deadlock up to three times (the sign-in and token paths, which touch
one row of their own, do not). Commands that name a staff member check that the author may assign roles at all before
