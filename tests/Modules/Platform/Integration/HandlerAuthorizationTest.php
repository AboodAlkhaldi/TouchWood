...
it('checks the right permission, against the right store, in every handler', function (Closure $run, string $permission, ?string $storeCode) {
    ...
    }, 'platform.media.upload', null],
    'change alt text' => [fn () => app(UpdateMediaAltTextHandler::class)->handle(new UpdateMediaAltText(mediaRowWithoutPermission(), 'مفصلة', 'Hinge')), 'platform.media.update', null],
    'delete media' => [fn () => app(DeleteMediaHandler::class)->handle(new DeleteMedia(mediaRowWithoutPermission())), 'platform.media.delete', null],
    'retry variants' => [fn () => app(RetryMediaVariantsHandler::class)->handle(new RetryMediaVariants(mediaRowWithoutPermission())), 'platform.media.upload', null],
    'generate variants' => [fn () => app(GenerateMediaVariantsHandler::class)->handle(new GenerateMediaVariants(mediaRowWithoutPermission('READY'))), 'platform.media.variants.generate', null],
    'mark variants failed' => [fn () => app(GenerateMediaVariantsHandler::class)->fail(new GenerateMediaVariants(mediaRowWithoutPermission('READY'))), 'platform.media.variants.generate', null],
    'sweep stuck variants' => [fn () => app(RequeueStuckMediaVariantsHandler::class)->handle(new RequeueStuckMediaVariants), 'platform.media.variants.generate', null],
]);
...
it('refuses every web request until Access exists, even though nobody has logged in', function () {
    expect(fn () => (new SystemOnlyAuthorizer(new SystemActorContext, runningInConsole: false))->authorize('platform.media.upload'))
