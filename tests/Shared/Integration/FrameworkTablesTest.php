...
| customer and staff tables (handoff §7.1). The tables for sessions, cache and queues stay: they run
| on PostgreSQL for now (owner, 2026-09-18). These tests stop the scaffolding from creeping back in.
*/
...
it('keeps the tables sessions, cache and queues run on', function (string $table) {
    expect(Schema::hasTable($table))->toBeTrue();
})->with(['sessions', 'cache', 'cache_locks', 'jobs', 'job_batches', 'failed_jobs']);

it('stores a ULID account id in the sessions table', function () {
